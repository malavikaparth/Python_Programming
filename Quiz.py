...
class Animals(Quiz):
    ...
    def getAnimalsQuestion(self, number):

        if number == 1:
            Animals.question1(self)
        elif number == 2:
            Animals.question2(self)
        elif number == 3:
            Animals.question3(self)
        elif number == 4:
            Animals.question4(self)
        elif number == 5:
            Animals.question5(self)
        else:
            print("Invalid choice. Please try again.")


class SolarSystem(Quiz):
    def __init__(self):
        Quiz.__init__(self)
        self.lives_used = 0
    ...
    # def getSportsQuestion(self, number):
    def getSolarSystemQuestion(self, number):

        if number == 1:
            SolarSystem.question1(self)
        elif number == 2:
            SolarSystem.question2(self)
        elif number == 3:
            SolarSystem.question3(self)
        elif number == 4:
            SolarSystem.question4(self)
        elif number == 5:
            SolarSystem.question5(self)
        else:
            print("Invalid choice. Please try again.")
...
class Main(Quiz):
    def __init__(self):
        ...
        self.food_seg = Food()
        self.animals_seg = Animals()
        self.solarsystem_seg = SolarSystem()
        self.quiz = Quiz()
        self.score = 0
        ...
        self.animals_questions_left = 5
        self.solarsystem_questions_left = 5
        ...
        self.animals_questions_answered = [False, False, False, False, False]
        self.solarsystem_questions_answered = [False, False, False, False, False]
        ...
        self.num_animals_ques = len(self.sp_questions_answered)
        self.num_solarsystem_ques = len(self.sp_questions_answered)
...
def category1(self):
    ...
    while self.sp_questions_left > 0:
        ...
        if self.quiz.lives <= self.sports_seg.lives_used + self.history_seg.lives_used + self.cultural_seg.lives_used + self.food_seg.lives_used + self.animals_seg.lives_used + self.solarsystem_seg.lives_used:
...
def category2(self):
    while self.h_questions_left > 0:
        ...
        if self.quiz.lives <= self.sports_seg.lives_used + self.history_seg.lives_used + self.cultural_seg.libes_used + self.food_seg.lives_used + self.animals_seg.lives_used + self.solarsystem_seg.lives_used:
            print("All lives lost.")
...
def category5(self):
    while self.animals_questions_left > 0:
        valid_choice = False
        while valid_choice == False:
            choice = int(input("\nChoose a question between 1 and 5: "))
            if self.animals_questions_answered[choice - 1] == True:
                print('This question has already been answered. Please choose another question')
                print('Questions left are:')
                for i in range(self.num_animals_ques):
                    if self.animals_questions_answered[i] == False:
                        print(i + 1)
                print()
            else:
                self.animals_questions_answered[choice - 1] = True
                self.animals_seg.getFoodQuestion(choice)
                self.questions_left -= 1
                self.animals_questions_left -= 1
                valid_choice = True

        if self.quiz.lives <= self.sports_seg.lives_used + self.history_seg.lives_used + self.cultural_seg.libes_used + self.food_seg.lives_used + self.animals_seg.lives_used + self.solarsystem_seg.lives_used:
            print("All lives lost.")
            self.animals_questions_left = 0
            self.questions_left = 0


def category6(self):
    while self.solarsystem_questions_left > 0:
        valid_choice = False
        while valid_choice == False:
            choice = int(input("\nChoose a question between 1 and 5: "))
            if self.solarsystem_questions_answered[choice - 1] == True:
                print('This question has already been answered. Please choose another question')
                print('Questions left are:')
                for i in range(self.num_solarsystem_ques):
                    if self.solarsystem_questions_answered[i] == False:
                        print(i + 1)
                print()
            else:
                self.solarsystem_questions_answered[choice - 1] = True
                self.solarsystem_seg.getFoodQuestion(choice)
                self.questions_left -= 1
                self.solarsystem_questions_left -= 1
                valid_choice = True

        if self.quiz.lives <= self.sports_seg.lives_used + self.history_seg.lives_used + self.cultural_seg.libes_used + self.food_seg.lives_used + self.animals_seg.lives_used + self.solarsystem_seg.lives_used:
            print("All lives lost.")
            self.solarsystem_questions_left = 0
            self.questions_left = 0
...
def quizMain(self):
    ...
    while self.questions_left > 0:
        ...
        print('6. SoalrSystem')
        print('7. Exit game')
        category = int(input("Choose a category (1-7): "))
    ...
    total_points = self.sports_seg.points_tally + self.history_seg.points_tally + self.cultural_seg.points_tally + self.food_seg.points_tally + self.animals_seg.points_tally + self.soalrsystem_seg.points_tally
